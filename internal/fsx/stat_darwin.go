...
// the platform-specific data isn't available (shouldn't happen on darwin).
...
func (f *File) DeviceIDs() (major, minor uint32) {
	...
	rdev := uint64(st.Rdev)
	// Read the second-to-last and last bytes of the big-endian device
	// number, rather than reimplementing the major()/minor() macros for
	// every dev_t layout in existence.
...
// CreatedTime returns the file's creation ("birth") time, if the
